...
export class ConverterComponent implements OnDestroy {
  ...
  model3dSampleMax = computed(() => {
    switch (this.model3dQualityLevel()) {
      case 'normal': return 500;    // 250K caras objetivo (buena calidad, rápido)
      case 'alta': return 1000;     // 500K caras objetivo (alta calidad, equilibrado)
      case 'maxima': return 2000;   // 1M caras objetivo (máxima calidad, lento)
      default: return 1000;
    }
  });

  // Computed para resolución volumétrica basada en calidad
  model3dVoxelResolution = computed(() => {
    switch (this.model3dQualityLevel()) {
      case 'normal': return 128;    // Resolución estándar
      case 'alta': return 256;      // Resolución alta
      case 'maxima': return 512;    // Resolución máxima
      default: return 256;
    }
  });

  // Computed para iteraciones de suavizado basado en calidad
  model3dSmoothingIterations = computed(() => {
    switch (this.model3dQualityLevel()) {
      case 'normal': return 3;      // Suavizado básico
      case 'alta': return 5;        // Suavizado mejorado
      case 'maxima': return 8;      // Suavizado máximo
      default: return 5;
    }
  });
  ...
  private getModel3DParams(): Model3DParams {
    return {
      ...
      wireframe: this.wireframe(),
      flatShading: this.flatShading(),
      targetFaces: Math.max(50000, this.model3dSampleMax() * 500), // Incrementado para mejores resultados
      voxelResolution: this.model3dVoxelResolution(), // Resolución basada en calidad
      smoothingIterations: this.model3dSmoothingIterations(), // Iteraciones basadas en calidad
      qualityLevel: this.model3dQualityLevel(), // Enviar nivel de calidad al backend
      manifold: true,
      ...
      Quality Level: ${this.model3dQualityLevel()}
      Target Resolution: ${this.model3dVoxelResolution()} voxels`,
