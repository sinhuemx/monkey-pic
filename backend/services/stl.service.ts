...
interface Model3DOptions {
  ...
  qualityLevel?: 'normal' | 'alta' | 'maxima';  // Nivel de calidad seleccionado
  voxelResolution?: number;                      // Resolución de voxels (128, 256, 512)
  smoothingIterations?: number;                  // Iteraciones de suavizado (3, 5, 8)
...
export class StlService {
  ...
  async generate3DModel(imageContent: Uint8Array, options: Model3DOptions = {}): Promise<{ stl?: string; obj?: string; error?: string, stdout?: string, stderr?: string }> {
    ...
    // NUEVOS PARÁMETROS DE CALIDAD - usar los enviados del frontend
    const qualityLevel = options.qualityLevel ?? 'alta';
    const voxelResolution = options.voxelResolution ?? 256;
    const smoothingIterations = options.smoothingIterations ?? 5;
    ...
    console.log(`  - Quality Level: ${qualityLevel} (Resolution: ${voxelResolution}, Smoothing: ${smoothingIterations})`);
    ...
    // Rutas limpias dentro del backend
    const pythonPath = "./ai3d/.venv/bin/python";
    const scriptPath = "./ai3d/volumetric_generator.py";
    
    const args = [
      scriptPath,  // MEJORADO: Ruta limpia para generador volumétrico
      "--input", tempImagePath,
      "--output", tempOutputPath,
      "--width", widthMM.toString(),
      "--depth", (widthMM * 0.75).toString(), // Depth proporcional al width
      "--height", maxHeightMM.toString(),
      "--resolution", voxelResolution.toString(), // Usar resolución real del frontend
      "--smoothing", smoothingIterations.toString(), // Usar iteraciones del frontend
      ...
      "--quality", qualityLevel, // Pasar nivel de calidad al script
    ];
    ...
    console.log(`Running 3D Model command: ${pythonPath} ${args.join(" ")}`);
  ...
  async generateHqModel(imageContent: Uint8Array, options: ModelOptions): Promise<{ stl?: string; obj?: string; error?: string, stdout?: string, stderr?: string }> {
    ...
    // Rutas limpias dentro del backend
    const pythonPath = "./ai3d/.venv/bin/python";
    const scriptPath = "./ai3d/estimate_and_mesh_optimized.py";
    
    const args = [
      scriptPath,  // MEJORADO: Ruta limpia para script optimizado
      "--input", tempImagePath,
      "--output", tempOutputPath,
    ...
    console.log(`Running HQ command: ${pythonPath} ${args.join(" ")}`);
